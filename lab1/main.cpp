...
int main()
{
    ...
    {
        outtextxy(100, 0, "欢迎使用NMSL Drawing Board Ver1.0！");
        outtextxy(170, 25, "1.创建一个圆对象");
        outtextxy(170, 50, "2.创建一个矩形对象");
        outtextxy(170, 75, "3.创建一个三角形对象");
        outtextxy(170, 100, "4.创建一条线对象");
        outtextxy(170, 125, "5.退出");
        k = getch();
        switch (k) {
        case 49: {
            cleardevice();
            char str[10], r[10], c[3], filled[2];
            inputbox_getline("请输入圆心的坐标：", "请输入圆心的坐标（例：（233，233））：（回车确认）", str, 100);
        ...
        case 50: {
            cleardevice();
            char p1[10],p2[10], c[3], filled[2];
            inputbox_getline("请输入矩形左上角顶点的坐标：", "请输入矩形左上角顶点的坐标（例：（233，233））：（回车确认）", p1, 10);
            inputbox_getline("请输入矩形右上角顶点的坐标：", "请输入矩形右下角顶点的坐标（例：（333，133））：（回车确认）", p2, 10);
            inputbox_getline("请输入矩形的颜色：", "请输入矩形的颜色（R代表红色，G代表绿色，B代表蓝色））：（回车确认）", c, 3);
            inputbox_getline("该矩形是否填充？：", "该圆是否填充？（填充请输入Y，不填充请输入N）：（回车确认）", filled, 2);
        ...
        case 51: {
            cleardevice();
            char p1[10],p2[10], p3[10],c[3], filled[2];
            inputbox_getline("请输入三角形第一个顶点的坐标：", "请输入三角形第一个顶点的坐标（例：（233，233））：（回车确认）", p1, 10);
            inputbox_getline("请输入三角形第二个顶点的坐标：", "请输入三角形第二个顶点的坐标（例：（333，400））：（回车确认）", p2, 10);
            inputbox_getline("请输入三角形第三个顶点的坐标：", "请输入三角形第三个顶点的坐标（例：（300，300））：（回车确认）", p3, 10);
        ...
        case 52: {
            cleardevice();
            char p1[10], p2[10], c[3];
            inputbox_getline("请输入线段第一个顶点的坐标：", "请输入线段第一个顶点的坐标（例：（233，233））：（回车确认）", p1, 10);
            inputbox_getline("请输入线段第二个顶点的坐标：", "请输入线段第二个顶点的坐标（例：（433，433））：（回车确认）", p2, 10);
